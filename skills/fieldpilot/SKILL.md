...
15. **Geography can change the answer.** Before a competitor, channel, pricing or regulatory conclusion, check whether geography could materially change the available alternatives, buyer access, platform usage, currency/price context or applicable constraints. If geography is known and material, load the matching local adapter. If it is unknown and material, keep `GEOGRAPHY_UNKNOWN`; do not silently substitute a global default or infer Korea from Korean-language input alone. Ask at most the minimum decision-changing geography question only when the conclusion cannot be bounded honestly without it. For Korea, apply the ordinary module's `GEOGRAPHY_AND_LOCAL_CONTEXT_GATE` (§2.3) and `KOREA_LOCAL_DISTRIBUTION.md`.
