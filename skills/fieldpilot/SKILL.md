...
15. **Geography changes the evidence set.** When target geography is known and materially changes competitors, regulation, pricing/currency, or audience access, load the matching local adapter before finalizing those parts of the decision. For Korea, apply `KOREA_LOCAL_DISTRIBUTION.md` and the conditional `REGULATORY_CONTEXT_CHECK.md` where regulation is material. Never infer the target market from the user's language, account location, or a familiar prior project. If geography is unresolved and can flip the recommendation, ask only that decision-changing question; otherwise keep the localized claim `UNKNOWN / COVERAGE_LIMITED`.
